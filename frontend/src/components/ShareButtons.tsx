...
import { pathToState, legacyKey } from '../routes'
...
function getMeta(url: string) {
  // 새 경로(/tax/...)와 레거시 쿼리(?view=) 모두 지원
  let pathname = url
  try {
    pathname = new URL(url, 'https://moajim.com').pathname
  } catch {
    pathname = url
  }
  const { view, sub } = pathToState(pathname)
  const key = legacyKey(view, sub)
  // 가장 구체적인 키 우선
  let best = ''
  for (const k of Object.keys(PAGE_META)) {
    if (key.includes(k) && k.length > best.length) best = k
  }
  if (best) return PAGE_META[best]
