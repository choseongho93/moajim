 ...
 */
import { pathToState, stateToPath, legacyKey } from '../routes'
...
export function updateSeoMeta() {
  // 경로 → (view, sub) → 레거시 키로 변환해 메타 테이블을 조회한다.
  const { view, sub } = pathToState(window.location.pathname)
  const query = legacyKey(view, sub)
  ...
  // canonical (경로 기반)
  const canonicalUrl = `${BASE_URL}${stateToPath(view, sub)}`
