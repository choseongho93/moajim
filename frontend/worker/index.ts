...
// ── 해외 접속 차단 (비용 보호) ──
// 한국(KR) IP만 허용. CF-IPCountry 헤더가 없으면(로컬 dev) 통과시킨다.
// undefined를 반환하면 다음 라우트로 넘어가고, Response를 반환하면 차단된다.
router.all('*', (request) => {
  const country = request.headers.get('CF-IPCountry') || ''
  if (country && country !== 'KR' && country !== 'T1') {
    return new Response(
      JSON.stringify({ error: 'This service is available in South Korea only.' }),
