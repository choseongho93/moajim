...
/** (view, sub) → 새 경로 (라우팅 단일 규칙, src/routes.ts와 동일) */
function stateToPath(view: string, sub: string): string {
  if (view === 'property-tax') return '/tax/property-tax'
  if (view === 'privacy') return '/tax/privacy'
  if (view === 'calculator' || view === 'tools' || view === 'finance') {
    return sub ? `/tax/${view}/${sub}` : `/tax/${view}`
  }
  return '/tax'
}

/** 레거시 ?view=&sub= → 새 경로. view 없으면 null */
function legacyQueryToPath(params: URLSearchParams): string | null {
  const view = params.get('view')
  if (!view) return null
  return stateToPath(view, params.get('sub') || '')
}

/** 새 경로 → SEO 조회용 레거시 키 (예: /tax/calculator/gift-tax → view=calculator&sub=gift-tax) */
function pathToLegacyKey(pathname: string): string {
  const segs = pathname.split('/').filter(Boolean)
  if (segs[0] !== 'tax' || segs.length === 1) return ''
  const section = segs[1]
  const sub = segs[2] || ''
  if (section === 'property-tax') return 'view=property-tax'
  if (section === 'privacy') return 'view=privacy'
  if (section === 'calculator' || section === 'tools' || section === 'finance') {
    return sub ? `view=${section}&sub=${sub}` : `view=${section}`
  }
  return ''
}

/** HTML에 SEO 메타 태그와 콘텐츠 주입 */
function injectSeo(html: string, seo: PageSeo, pathname: string): string {
...
export const onRequest: PagesFunction<{ ASSETS: Fetcher }> = async (context) => {
  ...
  // ── 해외 접속 차단 (비용 보호) ──
  // 한국(KR) IP만 허용한다. 단, 구글봇·빙봇·애드센스(mediapartners-google) 등
  // 검증 크롤러는 대부분 해외 IP에서 들어오므로 SEO/AdSense 유지를 위해 예외로 허용한다.
  // CF-IPCountry 헤더가 비어있는 경우(로컬 dev·프리뷰)는 차단하지 않는다.
  const country = context.request.headers.get('CF-IPCountry') || ''
  if (country && country !== 'KR' && country !== 'T1' && !isCrawler) {
    return new Response(
      '이 서비스는 대한민국에서만 이용할 수 있습니다.\nThis service is available in South Korea only.',
  ...
  // ── 레거시 URL 301 리다이렉트 (SEO 보존) ──
  // 기존 인덱싱된 ?view=&sub= → 새 경로(/tax/...)
  const legacyPath = legacyQueryToPath(url.searchParams)
  if (legacyPath) {
    return Response.redirect(new URL(legacyPath, url.origin).toString(), 301)
  }
  // 루트(/) → /tax
  if (url.pathname === '/') {
    return Response.redirect(new URL('/tax', url.origin).toString(), 301)
  }

  // index.html을 정적 에셋에서 직접 가져오기
  // (SPA 딥링크 폴백 + 크롤러 SEO 주입 공용)
  const assetUrl = new URL('/index.html', url.origin)
  const response = await context.env.ASSETS.fetch(assetUrl.toString())
  let html = await response.text()

  // 크롤러/SNS 미리보기 봇만 경로 기반 SEO 콘텐츠 주입
  // (일반 사용자는 원본 index.html 그대로 → FOUC 방지)
  if (isCrawler) {
    const seo = findSeo(pathToLegacyKey(url.pathname))
    html = injectSeo(html, seo, url.pathname)
  }
